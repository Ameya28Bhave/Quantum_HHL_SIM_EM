...
def simulateIQFT(psi, numQubits, rotations): 
    ...
    : The list of matrices needed for the algoithm.
    Returns:
...
def simulateQFT(psi, numQubits, invRotations): 
    ...
    for i in range(1, numQubits-1):
        ...
        for j in range(numQubits-2, i, -1):
            psi.ApplyControlled1QubitGate(j, i, invRotations[j-i-1]) # Return psi.
    return psi
...
def simulateQPE(psi, numQubits, qpeRotations, qftInvRotations): 
    ...
    : The number of qubits in the circuit.
    : The list of matrices needed for the algoithm.
    : The list of matrices needed for the IQFT.
        Returns:
    ...
    psi = simulateIQFT(psi, numQubits, qftInvRotations) # Return psi.
    return 
...
def simulateHHL(A, b, applyRYs, t, numQubits=4, extraAncillas=0, shots=2048,figfile='simulator.png', dpi=300):
    ...
    : The file name to save the histogram under.
    ...
    startTime = time.time() # For each shot:
    for i in range(shots):
        # Initialize a quantum system.
        psi = simulator.QubitRegister(numQubits + extraAncillas, 'base', initialSate, 0) # Intialize the target (last) qubit to the b state.
